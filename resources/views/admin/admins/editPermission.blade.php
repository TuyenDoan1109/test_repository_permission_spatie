...
@section('content')
    ...
    @php
    $permissionOfAdmin = [];
    foreach ($admin->getAllPermissions() as $item) {
        array_push($permissionOfAdmin, $item->id);
    }

    $permissionOfAdminViaRole = [];
    foreach ($admin->getPermissionsViaRoles() as $item) {
        array_push($permissionOfAdminViaRole, $item->id);
    }
    @endphp
                                ...
                                <label class="switch switch-3d switch-primary mr-3">
                                    <input data-filters="color" class="switch-input color-filter"
                                        @if(in_array($permission->id, $permissionOfAdmin))
                                            checked
                                        @endif
                                        @if(in_array($permission->id, $permissionOfAdminViaRole))
                                            disabled
                                        @endif
